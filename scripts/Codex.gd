...
var _icon_big_tex: TextureRect = null
...
func _on_select(rid: String) -> void:
	...
	if got:
		...
		if clean_path != "" and ResourceLoader.exists(clean_path):
			...
			if _icon_big_tex == null:
				_icon_big_tex = TextureRect.new()
				_icon_big_tex.name = "IconBigTexture"
				_icon_big_tex.custom_minimum_size = Vector2(0, 120)
				_icon_big_tex.expand_mode = TextureRect.EXPAND_FIT_WIDTH_PROPORTIONAL
				_icon_big_tex.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
				_icon_big.get_parent().add_child(_icon_big_tex)
				_icon_big.get_parent().move_child(_icon_big_tex, _icon_big.get_index())
			_icon_big_tex.visible = true
			_icon_big_tex.texture = load(clean_path)
		else:
			if _icon_big_tex:
				_icon_big_tex.visible = false
